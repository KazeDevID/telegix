...
export function isUploadableFile(value, key = '') {
  ...
  if (typeof value === 'object') {
    // If it's a LinkPreview or has link preview options properties, it's not a file
    if (
      value._options !== undefined ||
      value.prefer_small_media !== undefined ||
      value.prefer_large_media !== undefined ||
      value.show_above_text !== undefined ||
      value.is_disabled !== undefined
    ) {
      return false;
    }
    if ('source' in value) return true;
    if (typeof value.pipe === 'function') return true; // Stream
...
export async function normalizeFileSource(input, defaultFilename = 'file') {
  if (typeof input === 'string') {
    // If it's a file path on local filesystem (Node.js environment)
    try {
      if (fs && typeof fs.existsSync === 'function' && fs.existsSync(input)) {
        const buffer = await fs.promises.readFile(input);
        const filename = (path && typeof path.basename === 'function' && path.basename(input)) || defaultFilename;
        return {
      ...
      // Ignored in non-filesystem/edge environments
    }
    // Otherwise it's a file_id or URL string
    return { blob: input };
  }

  if ((typeof Buffer !== 'undefined' && typeof Buffer.isBuffer === 'function' && Buffer.isBuffer(input)) || input instanceof Uint8Array) {
    return {
      blob: new Blob([input]),
  ...
  if (typeof input === 'object') {
    ...
    if (input.source) {
      if (typeof input.source === 'string') {
        try {
          if (fs && typeof fs.existsSync === 'function' && fs.existsSync(input.source)) {
            const buffer = await fs.promises.readFile(input.source);
            return {
              blob: new Blob([buffer]),
              filename: input.filename || (path && typeof path.basename === 'function' && path.basename(input.source)) || defaultFilename,
            };
          }
        } catch {
          // Ignored
      ...
      if ((typeof Buffer !== 'undefined' && typeof Buffer.isBuffer === 'function' && Buffer.isBuffer(input.source)) || input.source instanceof Uint8Array) {
        return {
...
export function normalizeTelegramPayload(payload) {
  ...
  // 6. Handle link_preview_options and legacy disable_web_page_preview
  ...
  if (norm.disable_web_page_preview !== undefined && !norm.link_preview_options) {
...
export class Telegram {
  ...
  async call(method, payload = {}, options = {}) {
    ...
    if (hasUpload) {
      ...
      // Special Telegram API parameters that Telegram requires as JSON-serialized strings
  ...
  streamText(chatId, textStream, options = {}) {
    return runStreamText(this, chatId, textStream, options);
  }
